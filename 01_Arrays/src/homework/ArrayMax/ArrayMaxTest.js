...
function findAverageTimeTest(arr) {
	...
	const expected = arr.reduce((s, e) => s + e, 0) / arr.length;
	assert.strictEqual(actual, expected, 0.1);
}

function findMostProfitableClientTest(matrix) {
	const getAvg = arr => arr.reduce((s, e) => s + e, 0) / arr.length;
	const actual = findMostProfitableClient(matrix);
	const avgs = matrix.map(getAvg);
	const expected = avgs.indexOf(Math.max(...(avgs.filter(e => !isNaN(e)))));
	assert.strictEqual(actual, expected);
}
